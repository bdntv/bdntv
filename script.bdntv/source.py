...
if not token:
    ...
    urllib2.urlopen(str('http://bdncanada.com/bdntv/query/addclient.php?customer=%s&token=%s' % (customer.replace(' ', '%20'),token)))
...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.bdntv] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.bdntv] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.bdntv] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
                    ...
                    xbmcgui.Dialog().ok(ADDON.getAddonInfo('name'), 'string',
                                        'string', 'string')
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.bdntv] Updating caches...', xbmc.LOGDEBUG)
...
class XMLTVSource(Source):
    ...
    def __init__(self, addon):
        self.logoFolder = 'http://bdncanada.com/bdntv/guide/logos/'
        ...
        f = open(guideFile, 'wb')
        tmpData = urllib2.urlopen(str('http://bdncanada.com/bdntv/query/subscriptions.php?token=%s' % (token)))
        data = tmpData.read()
        f.write(data)
        f.close()
        self.xmltvFile = os.path.join(basePath, 'guide.xml')
...
def deleteDB():
    try:
        dbPath = xbmc.translatePath(xbmcaddon.Addon(id = 'script.bdntv').getAddonInfo('profile'))
