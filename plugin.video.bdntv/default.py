...
addon = xbmcaddon.Addon('plugin.video.bdntv')
...
SOURCES = [{"url": "http://dexteriptv.com/live/dex.php?kodi=a6105c0a611b41b08f1209506350279e", "fanart": "http://dexteriptv.com/live/thumbs/fanart.jpg", "genre": "Tv Live", "date": "01.09.2015", "credits": "DexterTV", "thumbnail": "http://1.bp.blogspot.com/-USW8oQI2iPA/T-FssXUqJ-I/AAAAAAAAAIU/eqk5G_L5fRY/s1600/dexter.png"}]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.video.bdntv-%s]: %s" %(addon_version, string))


def makeRequest(url, headers=None):
        try:
            if headers is None:
                headers = {'User-agent' : 'Zozilla/5.0 (Windows NT 6.1; WOW64; rv:19.0) Gecko/20100101 Zirefox/19.0'}
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(bdntv,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(bdntv,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(bdntv,New source added.,5000,"+icon+")")
        if not url is None:
            ...
            if k in regexs:
                ...
                if m['page'] and m['page'] in cachedPages and not 'ignorecache' in m and forCookieJarOnly==False :
                    ...
                    if m['page'] and  not m['page']=='' and  m['page'].startswith('http'):
                        ...
                        req.add_header('User-Agent', 'Zozilla/5.0 (Windows NT 6.1; rv:14.0) Gecko/20100101 Zirefox/14.0.1')
                        if 'refer' in m:
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(bdnTV,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(bdnTV,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('bdnTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('bdnTV', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        headers = dict({'User-Agent':'Zozilla/5.0 (Windows NT 6.3; rv:33.0) Gecko/20100101 Zirefox/33.0','Referer': 'http://joker.org/','Accept-Encoding':'gzip, deflate','Content-Type': 'application/json;charset=utf-8','Accept': 'application/json, text/plain, */*'})
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(bdnTV,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from channel bdnTV','XBMC.RunPlugin(%s?mode=6&name=%s)'
                ...
                contextMenu.append(('Add to bdnTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(bdnTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(bdnTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
