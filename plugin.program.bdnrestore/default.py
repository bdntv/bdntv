...
#Builds Section
def BuildMenu():
    link = OPEN_URL('http://bdncanada.com/bdnrestore/NewMenu.txt').replace('\n','').replace('\r','')
...
def BrettusMenu():
    link = OPEN_URL('http://shadowcrew.info/twins/Community_Builds/Brettus_Builds.txt').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,'wizard',iconimage,fanart,description)
    setView('movies', 'MAIN')
